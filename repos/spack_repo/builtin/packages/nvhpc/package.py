...
_versions = {
    "25.5": {
        "Linux-aarch64": (
            ...
            "https://developer.download.nvidia.com/hpc-sdk/25.5/nvhpc_2025_255_Linux_aarch64_cuda_multi.tar.gz",
        ),
        "Linux-x86_64": (
            "9a76cb93225b7f2e31a1894119f6e003a0562f52f43d2ca9a42ac08ecf2946d2",
            "https://developer.download.nvidia.com/hpc-sdk/25.5/nvhpc_2025_255_Linux_x86_64_cuda_multi.tar.gz",
        ),
        "single": {
            "Linux-aarch64": (
                "5378c53a38c2cd5efa3ee92c578c0656a8d341778619b9dfd4487dd636c5418b",
                "https://developer.download.nvidia.com/hpc-sdk/25.5/nvhpc_2025_255_Linux_aarch64_cuda_12.9.tar.gz",
            ),
            "Linux-x86_64": (
                "32ed33840bf38448bca346eff5efeb3f80796faa863e9ae0934d5745f2a561be",
                "https://developer.download.nvidia.com/hpc-sdk/25.5/nvhpc_2025_255_Linux_x86_64_cuda_12.9.tar.gz",
            ),
        },
    },
    # "25.3": {
...
class Nvhpc(Package, CompilerPackage):
    ...
    for ver, packages in _versions.items():
        version(ver) # , url="https://github.com/pika-org/pika/archive/refs/tags/0.34.0.tar.gz") #, sha256="a5c5c8726d2210f2310a852c6d6e03c9ef8c75e3643e9c94e2490aaaaaaaea7a")
        ...
        if pkg:
            resource(
                name="nvhpc",
                when=f"@{ver} +multi_cuda",
                sha256=pkg[0],
                url=pkg[1],
                placement="nvhpc",
            )

        single_cuda = packages.get("single")
        if single_cuda:
            pkg = single_cuda.get(key)
            if pkg:
                resource(
                    name="nvhpc",
                    when=f"@{ver} ~multi_cuda",
            ...
            conflicts(
                "~multi_cuda",
                when=f"@{ver}",
                msg=f"This version of NVHPC can only be installed with +multi_cuda, i.e. multiple CUDA versions",
            )

    depends_on("gcc languages=c,c++,fortran", type="run")

    variant("multi_cuda", default=True, description="Install NVHPC providing multiple CUDA versions; when disabled installs only a single CUDA version")
    ...
    def install(self, spec, prefix):
        with working_dir("nvhpc"):
            filter_file(r"#! /bin/bash", "#!/usr/bin/env -S bash -o xtrace", "install")
            filter_file(r"#! /bin/sh", "#!/usr/bin/env -S bash -o xtrace", "install_components/install")
