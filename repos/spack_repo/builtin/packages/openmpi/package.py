...
class Openmpi(AutotoolsPackage, CudaPackage):
    ...
    # Current
    version("5.0.8", sha256="53131e1a57e7270f645707f8b0b65ba56048f5b5ac3f68faabed3eb0d710e449")
    version(
...
'-Wl,-flat_namespace'.""",
    ...
    def with_or_without_xpmem(self, activated):
        s = "xpmem" if self.spec.satisfies("~cray-xpmem") else "cray-xpmem"
        if not activated:
            return f"--without-{s}"
        return f"--with-{s}={self.spec['xpmem'].prefix}"
